...
static void drawTerrain()
{
	...
	uint16 i,j;
	for(i=paintRegionX1;i<=paintRegionX2+1;i++)
	{
		Vec3D point1;
		Vec3D point2;
	...
	for(i=paintRegionY1;i<=paintRegionY2+1;i++)
	{
		Vec3D point1;
		Vec3D point2;
		point1[1] = point2[1] = -0.5;
	...
	for(i=paintRegionX1;i<=paintRegionX2;i++)
	{
		for(j=paintRegionY1;j<=paintRegionY2;j++)
		{
			if(snakeGame.map[i][j]==MAP_WALL_HARD)
			{
				...
				for(p=0;p<4;p++)
				{
					...
					temp[1] = -0.5; 
					OLEDFB3D_drawLine3D(quad[p],temp);
				}
			}
...
static void drawSnake()
{
	...
	uint16 i,j;
    for(i=0;i<= snakeGame.snakeLength;i++)
    {
    	...
    	else if (i == snakeGame.snakeLength)
    	{
    		...
    		for(p=0;p<3;p++)
    		{
    			OLEDFB3D_drawLine3D(currentPoint[0],lastPoint[p]);
    		}
    	}
    	else
    	{
    		...
    		if(i==1)
    		{
        		for(j=0;j<3;j++)
        		{
        			OLEDFB3D_drawLine3D(lastPoint[0],currentPoint[j]);
        		}
    		}
    		else
    		{
				for(j=0;j<3;j++)
				{
					OLEDFB3D_drawLine3D(lastPoint[j],currentPoint[j]);
				}
    		}
    		for(j=0;j<3;j++)
    		{
    			...
    			for(k=0;k<j;k++)
    			{
    				OLEDFB3D_drawLine3D(currentPoint[j],currentPoint[k]);
    			}
    		}
    	}
		...
		for(j=0;j<3;j++){
			uint16 k;
			for(k=0;k<3;k++)
				lastPoint[j][k] = currentPoint[j][k];
		}
...
{	
	...
	{
	case STATE_SPLASH:
		OLEDFB_drawTextEx(5,5,24,24, "ekanS");
		OLEDFB_drawTextEx(100,29,12,12, "D3");
		OLEDFB_drawTextEx(0,45,8,8, "EUNITNOC>-YEKYNA");
		break;
	case STATE_GAME:
		drawTerrain();
		drawSnake();
	...
	case STATE_GAMEOVER:
		{
			char temp[20];
			sprintf(temp,"%d:erocS",score);
			OLEDFB_drawTextEx(5,5,12,12, "revOemaG");
			uint16 i;
			for(i=0;i<8;i++)
			{
				if(i>=2)
...
static void animationTimerHandler()
{
	...
	else if(state == STATE_GAME)
	{
		...
		{
			if(lastPressedKey == KEY_DOWN) snakeDirection = SNAKE_DIRECTION_DOWN;
			else if(lastPressedKey == KEY_UP) snakeDirection = SNAKE_DIRECTION_UP;
			...
			uint16 i,j;
			for(i=snakeGame.snakeLength;i>=1;i--)
			{
				for(j=0;j<2;j++)
				{
			...
			for(i=1;i<snakeGame.snakeLength;i++)
			{
...
static void eventHandler(int event, int data)
{
	...
	case EVENT_KEY_UP:
		...
		else if(state == STATE_GAME)
		{
			...
			if(lastPressedKey == KEY_DOWN && snakeDirection == SNAKE_DIRECTION_DOWN) lastPressedKey = -1;
			if(lastPressedKey == KEY_UP && snakeDirection == SNAKE_DIRECTION_UP) lastPressedKey = -1;
